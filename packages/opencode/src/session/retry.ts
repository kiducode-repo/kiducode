...
export const GO_UPSELL_URL = "https://kiducode.com/go"
...
export function retryable(error: Err, provider: string) {
  ...
  if (MessageV2.APIError.isInstance(error)) {
    ...
    if (error.data.responseBody?.includes("FreeUsageLimitError")) {
      return {
        ...
        action: {
          ...
          message: "Subscribe to KiduCode Go for reliable access to the best open-source models, starting at $5/month.",
          label: "subscribe",
          link: GO_UPSELL_URL,
        },
      }
    ...
    if (error.data.responseBody?.includes("GoUsageLimitError")) {
      ...
      const link = `https://kiducode.com/workspace/${workspace}/go`
      return {

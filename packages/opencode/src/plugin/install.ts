...
export type PatchDeps = {
  ...
  files: (dir: string, name: "kiducode" | "opencode" | "tui") => string[]
}
...
function patchDir(input: PatchInput) {
  ...
  return path.join(root, ".kiducode")
}

function patchName(kind: Kind): "kiducode" | "tui" {
  if (kind === "server") return "kiducode"
  return "tui"
}

...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  // Every config dir we may read from: global config dir, any `.kiducode`
  // or legacy `.opencode` folders between cwd and home, and OPENCODE_CONFIG_DIR.
  ...
  // 4. `.kiducode`/`.opencode` directories (and OPENCODE_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter(
    (dir) => dir.endsWith(".kiducode") || dir.endsWith(".opencode") || dir === Flag.OPENCODE_CONFIG_DIR,
  )

  for (const dir of dirs) {
    if (!dir.endsWith(".kiducode") && !dir.endsWith(".opencode") && dir !== Flag.OPENCODE_CONFIG_DIR) continue

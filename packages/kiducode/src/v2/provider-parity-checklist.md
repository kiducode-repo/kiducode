...
This tracks legacy provider behavior from `packages/kiducode/src/provider/provider.ts` that still needs to be ported into the v2 provider plugins under `packages/kiducode/src/v2/plugin/provider/`. Keep entries checked only when v2 has equivalent behavior or when the item is intentionally skipped.

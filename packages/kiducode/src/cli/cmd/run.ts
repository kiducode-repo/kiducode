// CLI entry point for `kiducode run`.
//
// Handles three modes:
...
//      kiducode server and runs interactive mode against it.
//
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run kiducode with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running kiducode server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",

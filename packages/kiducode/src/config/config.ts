...
function normalizeLoadedConfig(data: unknown, source: string) {
  ...
  delete copy.tui
  log.warn("tui keys in KiduCode config are deprecated; move them to tui.json", { path: source })
  return copy
}
...
export const Info = Schema.Struct({
  ...
  server: Schema.optional(ConfigServer.Server).annotate({
    description: "Server configuration for kiducode serve and web commands",
  }),
  command: Schema.optional(Schema.Record(Schema.String, ConfigCommand.Info)).annotate({
    description: "Command configuration, see https://kiducode.com/docs/commands",
  }),
  ...
  ).annotate({ description: "Agent configuration, see https://kiducode.com/docs/agents" }),
...
function globalConfigFile() {
  const candidates = ["kiducode.jsonc", "kiducode.json", "opencode.jsonc", "opencode.json", "config.json"].map((file) =>
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadConfig = Effect.fnUntraced(function* (
      ...
      if (!data.$schema) {
        data.$schema = "https://kiducode.com/config.json"
        const updated = text.replace(/^\s*\{/, '{\n  "$schema": "https://kiducode.com/config.json",')
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      if (!Flag.OPENCODE_CONFIG && !Flag.OPENCODE_CONFIG_DIR && !Flag.OPENCODE_CONFIG_CONTENT) {
        ...
        if (!existsSync(file)) {
          yield* fs
            .writeWithDirs(file, JSON.stringify({ $schema: "https://kiducode.com/config.json" }, null, 2))
      ...
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "kiducode.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "kiducode.jsonc"), env))
        ...
        yield* Effect.promise(() =>
          import(pathToFileURL(legacy).href, { with: { type: "toml" } })
            .then(async (mod) => {
              ...
              result["$schema"] = "https://kiducode.com/config.json"
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "KIDUCODE_CONFIG_CONTENT" || source === "OPENCODE_CONFIG_CONTENT") return "local"
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            if (!remoteConfig.$schema) remoteConfig.$schema = "https://kiducode.com/config.json"
        ...
        if (!Flag.OPENCODE_DISABLE_PROJECT_CONFIG) {
          ...
          for (const file of yield* ConfigPaths.files("kiducode", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
            yield* merge(file, yield* loadFile(file, authEnv), "local")
          }
        }
        ...
        for (const dir of directories) {
          if (dir.endsWith(".kiducode") || dir.endsWith(".opencode") || dir === Flag.OPENCODE_CONFIG_DIR) {
            for (const file of ["opencode.json", "opencode.jsonc", "kiducode.json", "kiducode.jsonc"]) {
          ...
          // Auto-discovered plugins under `.kiducode/plugin(s)` or legacy `.opencode/plugin(s)` are already local files, so ConfigPlugin.load
          // returns normalized Specs and we only need to attach origin metadata here.
          const list = yield* Effect.promise(() => ConfigPlugin.load(dir))
          yield* mergePluginOrigins(dir, list)
        }

        const configContent = process.env.KIDUCODE_CONFIG_CONTENT ?? process.env.OPENCODE_CONFIG_CONTENT
        if (configContent) {
          const source = process.env.KIDUCODE_CONFIG_CONTENT ? "KIDUCODE_CONFIG_CONTENT" : "OPENCODE_CONFIG_CONTENT"

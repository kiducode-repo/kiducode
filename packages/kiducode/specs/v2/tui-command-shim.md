...
Delete `packages/kiducode/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/kiducode/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/kiducode/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/kiducode`
- TUI plugin loader tests in `packages/kiducode` if runtime plugin API wiring changed

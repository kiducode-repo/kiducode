...
- The server still lives in `packages/kiducode`.
...
Until enough shared service code lives outside `packages/kiducode`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/kiducode`

It should not import `packages/kiducode` services while `packages/kiducode`
...
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/kiducode` implementation details.

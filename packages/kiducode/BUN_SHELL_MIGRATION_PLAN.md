...
Replace runtime Bun shell template-tag usage in `packages/kiducode/src` with a unified `Process` API in `util/process.ts`.
...
- Runtime Bun `$` usage in `packages/kiducode/src` is removed except:
